...
const LibraryView: React.FC = () => {
  ...
  const getThoughtStatus = (thought: Thought): { type: 'share' | 'todo' | 'idea' | 'shared'; label: string; color: string } => {
    ...
    if (thought.potential === 'Do') {
      const isComplete = thought.todoData?.completed;
      return { type: 'todo', label: isComplete ? 'Done' : 'To-do', color: isComplete ? 'emerald' : 'amber' };
    }
  ...
  const statusColors: Record<string, { bg: string; text: string; border: string; icon: string }> = {
    violet: { bg: 'bg-violet-50', text: 'text-violet-700', border: 'border-l-violet-400', icon: 'text-violet-500' },
    amber: { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-l-amber-400', icon: 'text-amber-500' },
    emerald: { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-l-emerald-400', icon: 'text-emerald-500' },
    slate: { bg: 'bg-slate-50', text: 'text-slate-600', border: 'border-l-slate-300', icon: 'text-slate-400' },
  };
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-slate-50 rounded-xl border-0 text-base focus:outline-none focus:ring-2 focus:ring-violet-500/30 focus:bg-white transition-all"
          />
        </div>
        ...
        <div className="flex items-center justify-between mb-6 text-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-slate-600 hover:bg-slate-100 rounded-lg transition-all"
            >
          ...
          <div className="space-y-2">
            {filteredThoughts.map((thought) => {
              ...
              return (
                <div
                  key={thought.id}
                  className={`group rounded-lg border-l-4 ${colors.border} bg-white shadow-sm hover:shadow transition-all ${
                  ...
                  {isEditing ? (
                    <div className="p-4">
                      <textarea
                        ...
                        className="w-full p-3 bg-slate-50 rounded-lg border border-slate-200 text-slate-800 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-violet-500/20"
                        rows={4}
                        autoFocus
                      />
                    ...
                    <div className="p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className={`mt-0.5 ${colors.icon}`}>
                          {status.type === 'share' && <Sparkles className="w-4 h-4" />}
                          {status.type === 'todo' && <ListTodo className="w-4 h-4" />}
                          {status.type === 'shared' && <CheckCircle className="w-4 h-4" />}
                          {status.type === 'idea' && <div className="w-4 h-4 rounded-full bg-slate-300" />}
                        </div>
