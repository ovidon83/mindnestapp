...
const MindboxView: React.FC = () => {
  ...
  const getBadgeInfo = (type: EntryType) => {
    switch (type) {
      ...
      case 'insight':
        return { label: 'Insight', icon: Lightbulb, color: 'teal' };
      case 'journal':
                  ...
                  <div className="flex-shrink-0 flex items-start justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {entry.postingScore !== undefined && entry.postingScore >= 50 && !entry.inShareIt && (
                        <div className="px-2.5 py-1 rounded-lg text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200 flex items-center gap-1.5">
                          <Share2 className="w-3 h-3" />
                          <span>Worth sharing</span>
                          <span className="text-amber-600">({entry.postingScore})</span>
                        </div>
                      )}
                    </div>
